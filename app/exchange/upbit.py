...
def buy(symbol):
    ...
    print("Now:", datetime.datetime.now(), 'Balance:', krw, 'Buy Price:', sell_price, 'Shares:', unit)
...
def sell(symbol):
    ...
    unit = float(krw) / float(buy_price)
    #upbit_api.sell_limit_order(symbol, buy_price, unit)
    print("Now:", datetime.datetime.now(), 'Balance:', krw, 'Sell Price:', buy_price, 'Shares:', unit)
